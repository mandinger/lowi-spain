...
- Session/SSO longevity in practice (how long an idle session survives before even the silent refresh below stops working). On a session failure, the integration replays `GET /milowi/login/` (confirmed by a captured browser reload to be how Django/Keycloak silently recognize a returning session - no `prompt=none` parameter involved) before falling back to Home Assistant's interactive reauth (see `docs/lowi-auth-and-api.md` §5, §8) — how long that silent path keeps working unattended isn't measured yet.
